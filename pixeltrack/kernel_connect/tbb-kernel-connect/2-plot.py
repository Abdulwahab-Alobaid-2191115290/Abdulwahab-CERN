...
for filename in os.listdir(directory):
    ...
    if filename.endswith('.csv'):
        ...
        if i%2 != 0:
            ...
            ax2.plot(Threads, Time, marker="o", label=f"thresh-{Thresh}")
            #ax2.errorbar(Threads, Time, fmt='o', yerr=Stdev, ms=0, capsize=10)
        i += 1


# Titles
ax1.set_title("Time of Execution kernel_connect() with increasing ThreshHold and NumberOfThreads On The TBB Backend")
ax2.set_title("Time of Execution kernel_connect() with increasing ThreshHold and NumberOfThreads On The TBB Backend")
