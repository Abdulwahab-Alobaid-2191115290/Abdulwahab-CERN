...
def avg_stdev(filename):
    ...
    # for each line on the form "pre kernel post"
    for r in readings:
        arr = r.split()
    ...
    # return [ kernel stdev, avg pre memory, avg kernel time, avg post memory ]
...
for key in files_d:
    ...
    for filename in files_d[key]:
                                                    #   idx  :         0              1              2                 3
        stats = avg_stdev(key + '/' + filename)     # returns: [ kernel-stdev, avg-pre-memory, avg-kernel-time, avg-post-memory ]
        time = stats[2]
        stdev = stats[0]
        size = filename[:filename.find("x")]        # extracts size from filename
